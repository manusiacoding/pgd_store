    ...
    <nav class="header-navbar navbar navbar-expand-lg align-items-center floating-nav navbar-light navbar-shadow">
        <div class="navbar-container d-flex content">
            ...
            <ul class="nav navbar-nav align-items-center ml-auto">
                ...
                @php
                    $user = App\Models\User::with('profilePhoto')->where('id', Auth::user()->id)->first();
                @endphp
                <li class="nav-item dropdown dropdown-user"><a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <div class="user-nav d-sm-flex d-none"><span class="user-name font-weight-bolder">{{ Auth::user()->name }}</span></div><span class="avatar">
                            @if (is_null($user->profilePhoto))
                                <img class="round" src="https://avatars.dicebear.com/4.5/api/initials/{{ Auth::user()->name }}.svg" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>
                            @elseif (is_null($user->profilePhoto->foto))
                                <img class="round" src="https://avatars.dicebear.com/4.5/api/initials/{{ Auth::user()->name }}.svg" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>
                            @elseif (!is_null($user->profilePhoto->foto))
                                <img class="round" src="{{ Storage::url($user->profilePhoto->foto) }}" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>
                            @endif
                    </a>
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-user">
                        @role('user')
                            ...
                            <a class="dropdown-item" href="{{ route('user.dashboard.settings') }}">
