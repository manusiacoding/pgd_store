...
jQuery('.se-pre-con a').click(function( event ) {
    event.preventDefault();
        // Do other stuff if needed
        });
        jQuery('.se-pre-con .btn').click(function( event ) {
            event.preventDefault();
                // Do other stuff if needed
                });
...
function notificationCart(elem){
    var id_produk = jQuery(elem).attr("id");

    jQuery.ajax({
        type: 'GET',
        url: "http://localhost:8004/cart/" + id_produk,
...
function notificationWishlist(elem){

    var id_produk = jQuery(elem).attr("id");

    jQuery.ajax({
        type: 'GET',
        url: "http://localhost:8004/wishlist/" + id_produk,
        success:function(data){
...
jQuery(document).on("click","#switchColor a", function(){
    ...
    if(cssValue == "default"){
        ...
        // if(pagename == 'index-5.html' || pagename == 'index-6.html' || pagename == 'index-7.html' || pagename == 'index-8.html' || pagename == 'index-9.html'  ){
        //     jQuery( "body" ).addClass( "home5" );
        // }
        ...
        for(var i=5 ; i< 10; i++){
            if(pagename === `index-${i}.html`){
                jQuery( "body" ).removeClass( `home${i}` );
            }
        }
...
jQuery(window).on('load',function(){
    ...
    setTimeout(function(){jQuery('#alert-cookie').show();
    }, 2000);

    setTimeout(function(){jQuery('#alert-cookie5').show();
    }, 2000);

    setTimeout(function(){
        jQuery('#alert-cookie5a').show(); }, 3000);

    setTimeout(function(){
    jQuery('#alert-cookie2').show(); }, 4000);

    setTimeout(function(){
        jQuery('#alert-cookie6').show(); }, 3000);
    setTimeout(function(){
        jQuery('#alert-cookie7').show(); }, 3000);
    setTimeout(function(){
        jQuery('#alert-cookie8').show(); }, 3000);
        setTimeout(function(){jQuery('#alert-cookie8').show();
    }, 2000);
